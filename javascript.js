...
async function handleNewDelivery(e) {
    e.preventDefault();
    const form = e.target;
    // O new FormData(form) JÁ apanha todos os campos,
    // incluindo o <input name="clientId"> escondido.
    const formData = new FormData(form);

    // --- (A CORREÇÃO ESTÁ AQUI) ---
    // As linhas abaixo foram REMOVIDAS porque estavam a duplicar o clientId,
    // o que o transformava num Array e causava o erro no backend.
    /*
...
async function openHistoryDetailModal(orderId) {
    ...
    try {
        ...
        body.innerHTML = `
            ...
            <p><strong>Criado em:</strong> ${new Date(order.createdAt).toLocaleString('pt-MZ')}</p>
...
/* --- Funções para Extrato de Cliente --- */
