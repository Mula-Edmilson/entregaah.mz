...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (adminDashboard) {
        ...
        freeIcon = L.icon({
            iconUrl: 'https://unpkg.com/leaflet@1.9.4/dist/images/marker-icon.png', // Corrigido
            shadowUrl: iconShadowUrl,
            iconSize: [25, 41], iconAnchor: [12, 41], popupAnchor: [1, -34], shadowSize: [41, 41]
        });
        
        busyIcon = L.icon({
            iconUrl: 'https://i.postimg.cc/J0bJ0fJj/marker-busy.png', // Este estava bom
            shadowUrl: iconShadowUrl, // Corrigido
            iconSize: [25, 41], iconAnchor: [12, 41], popupAnchor: [1, -34], shadowSize: [41, 41]
        });
        // --- FIM DA CORREÇÃO ---
...
async function loadActiveDeliveries() {
    try {
        ...
        if (data.orders.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="7">Nenhuma encomenda ativa.</td></tr>'; // Colspan 7
            return;
        }
...
async function loadHistory() {
    try {
        ...
        tableBody.innerHTML = '';
        if (data.orders.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="7">Nenhum histórico encontrado.</td></tr>'; // Colspan 7
            return;
        }
        data.orders.forEach(order => {
            const motoristaNome = order.assigned_to_driver ? order.assigned_to_driver.user.nome : 'N/D';
...
function showDetalheEntrega(order) {
    ...
    if (order.address_coords && order.address_coords.lat) {
        ...
        // (LINK CORRIGIDO)
        mapButton.href = `http://googleusercontent.com/maps.google.com/6{order.address_coords.lat},${order.address_coords.lng}`;
