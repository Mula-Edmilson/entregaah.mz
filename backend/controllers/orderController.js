...
      if (driverProfile) {
        io.to(driverProfile.user.toString()).emit('entrega_cancelada', { orderId: order._id });
      }
    } catch (socketError) {
      console.error('Erro ao notificar motorista sobre cancelamento:', socketError);
    }
  }

  res.status(200).json({ message: 'Encomenda cancelada com sucesso.', order });
});
